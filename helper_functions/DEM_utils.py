...
def get_DEM():
    ...
    return SG_DEM_raster_arr, geotransform_SG_DEM_raster
...
def get_DEM_value(lon,lat,geotransform):
    """
    Args:
        lon (float): longitude in degrees decimals
        lat (float): latitude in degrees decimals
        geotransform (tuple): 6 tuple that describes pixel size and lon lat resolution
    Returns:
        dx (int): column pixel from the origin (upper left corner)
        dy (int): row pixel from the origin (upper left corner)
    """
