...
$$
Y_{it} = \lambda_i + \delta_t + Treat_i + \sum_{k\geq-12, k\neq-1}^{k=6}D_{it}^k + \sum_{k\geq-12, k\neq-1}^{k=6}(\beta_kTreat_i \times D_{it}^k) + \epsilon_{it} 
$$
...
bind_rows(buffer_df_list, .id="id")%>%
  select(id,period_D)%>%
  mutate_at(vars(id), function(x)
    str_replace_all(x,c("dsmall"="Treat=","dbig"=", Control=")))%>%
...
local_DID_df_list <- lapply(seq_along(buffer_df_list), function(i){
  ...
  model_property <- get_event_study_df(buffer_df_list[[i]], period_D_list = seq(-12,6,1), base_period=-1)
...
write.csv(local_DID_df_list,
          file.path(buffer_dir,"processed_outputs",
                        sprintf("episodicFlood_stackedDID_EventStudy_%s.csv","potentialcontamination_removed")
                        ),
            row.names = FALSE)
local_DID_df_list
```

## 2.3 Plot event study (robustness)
```{r}
model_names <- c("contaminated","contaminated_removed", "potentialcontamination_removed")

sapply(model_names, function(x){
  plot_event_study_robustness(fp = file.path(buffer_dir,"processed_outputs",
                        sprintf("episodicFlood_stackedDID_EventStudy_%s.csv",x)
                        ),

                        save_fp=file.path(getwd(),"Exported_Data","exported_figures",
                        sprintf("episodicFlood_stackedDID_EventStudy_%s.svg",x)
                        )
                        )
})
```
## 2.3 Plot event study (estimates)

```{r}
model_names <- c("contaminated","contaminated_removed", "potentialcontamination_removed")

sapply(model_names, function(x){
  plot_event_study_estimates(fp = file.path(buffer_dir,"processed_outputs",
                        sprintf("episodicFlood_stackedDID_EventStudy_%s.csv",x)
                        ),

                        save_fp=file.path(getwd(),"Exported_Data","exported_figures",
                        sprintf("episodicFlood_stackedDID_EventStudyEstimate_%s.svg",x)
                        )
                        )
})
```

# 3 Treatment Heterogeneity via adaptation likelihood

```{r}
fn <- "PrivRes_20260403_dsmall170dbig240_networkDepthNone.csv"
fp_list <- list(file.path(getwd(),"Exported_Data","adaptation",fn))
import_files(fp_list)
```
...
# filter property type so we focus analysis on different panels
local_DID_df <- buffer_df_list$dsmall170dbig240%>%
  filter(Property_Type == property_type)%>%
  # filter(contaminated_rows==0)
  filter(potential_contamination==0)
...
summary(model_property)
results_df <- get_model_results(model_property, model_name = "Treat170_Control240")
